...
const DECK_ID = "project-ppt";
const OUT_DIR = "/Users/ashoksinghdhami/Documents/New project/outputs/project-ppt";
const REF_DIR = "/Users/ashoksinghdhami/Documents/New project/tmp/slides/project-ppt/pro-reference-images";
...
const SOURCES = {
  primary: "Local project workspace: README.md, PROJECT_NOTES.md, app.js, server.js, extension/*",
  live: "Live deployment: https://miniproject-rjwq.onrender.com",
  repo: "GitHub repository: https://github.com/badash01/miniproject",
};

const SLIDES = [
  {
    "kicker": "MCA MINI PROJECT",
    "title": "Research Paper Summariser",
    "subtitle": "An AI-powered web app and Chrome extension that turns long research papers into fast, structured study notes.",
    "expectedVisual": "Cover slide with strong title block and one project-positioning panel.",
    "moment": "Web app + Chrome extension + LLM backend",
    "notes": "Open by saying the project helps students quickly understand difficult papers without manually reading every section in detail.",
    "sources": [
      "primary",
      "live"
    ]
  },
  {
    ...
    "subtitle": "Research papers are long, technical, and time-consuming for students to read, compare, and convert into exam-ready notes.",
    "expectedVisual": "Three problem cards showing pain points faced by students and researchers.",
    "cards": [
      [
        "Reading overload",
        "A single paper may contain abstract, methodology, experiments, results, and references across many dense pages."
      ],
      [
        "Manual note-making",
        "Students often spend more time extracting key points than understanding the core idea of the paper itself."
      ],
      [
        "Too much context switching",
        "Users jump between paper websites, PDFs, browser tabs, copied text, and separate note documents."
      ]
    ],
    "notes": "Explain that the project focuses on speed, clarity, and reducing friction during paper study.",
    "sources": [
      "primary"
    ]
  },
  {
    ...
    "subtitle": "The system gives users two simple entry points for summarising papers and one secure backend for running the LLM request.",
    "expectedVisual": "Three factual metric cards describing product surfaces and deployment.",
    "metrics": [
      [
        "2",
        "User interfaces",
      ...
      [
        "1",
        "Secure backend",
        "Node.js API hides the model key"
      ],
      [
        "Free",
        "Cloud deployment",
    ...
    "notes": "Describe the product in one line: give paper text, get a structured summary with title, keywords, contributions, methodology, findings, and notes.",
    "sources": [
      "primary",
      "live"
    ]
  },
  {
    ...
    "subtitle": "Frontend clients send research text to a Node backend, which securely calls the LLM provider and returns structured JSON.",
    "expectedVisual": "Architecture-style cards from input layer to output layer.",
    "cards": [
      [
        "Input layer",
        "Paste text, upload TXT or PDF, or let the Chrome extension auto-detect a paper from the current website."
      ],
      [
        "Backend layer",
        "The Node server validates the request, loads environment variables, builds the prompt, and calls the model endpoint."
      ],
      [
        "Output layer",
        "The app displays title, keywords, abstract summary, contributions, methodology, findings, and study notes."
      ]
    ],
    "notes": "Make the security point clearly: the browser never directly exposes the API key.",
    "sources": [
      "primary",
      "repo"
    ]
  },
  {
    ...
    "subtitle": "The browser experience is designed for direct paper input and quick summarisation without technical setup from the user side.",
    "expectedVisual": "Three-step flow cards for the website path.",
    "cards": [
      [
        "Step 1: Provide content",
        "The user uploads a PDF or TXT file, or directly pastes a research paper abstract, introduction, or full text."
      ],
      [
        "Step 2: Summarise",
        "The frontend sends the cleaned research text to the backend endpoint `/api/summarize` with the chosen summary length."
      ],
      [
        "Step 3: Review output",
        "The page renders concise, sectioned results so the user can quickly revise the paper or prepare for a viva."
      ]
    ],
    "notes": "Mention the fallback summariser in the web app as a backup path if the LLM call fails.",
    "sources": [
      "primary"
    ]
  },
  {
    ...
    "subtitle": "The extension removes copy-paste friction by scanning the current page and detecting research-paper content automatically.",
    "expectedVisual": "Three factual metric cards tied to extension capabilities.",
    "metrics": [
      [
        "Auto",
        "Paper detection",
        "Reads metadata, abstract, article, and main content"
      ],
      [
        "1 click",
        "Summary trigger",
        "Summarise from the popup"
      ],
      [
        "Live",
        "Works on websites",
        "Best on HTML abstract and journal pages"
      ]
    ],
    "notes": "Explain that the extension uses active tab access and page scanning logic, but still sends the final text through the same backend.",
    "sources": [
      "primary"
    ]
  },
  {
    "kicker": "SECTION 07",
    "title": "Tech Stack and Security Choices",
    "subtitle": "The project uses one JavaScript stack across frontend, backend, and extension, with the secret key isolated on the server side.",
    "expectedVisual": "Technology and security tradeoff cards.",
    "cards": [
      [
        "Frontend",
        "HTML, CSS, and JavaScript power the browser UI, file input flow, PDF parsing, and result rendering."
      ],
      [
        "Backend",
        "Node.js handles environment variables, API routes, prompt creation, and the call to the selected LLM provider."
      ],
      [
        "Security",
        "The API key is never placed in frontend code; it stays in environment variables and is used only by the server."
      ]
    ],
    "notes": "This slide is a good place to answer why the backend is necessary and why environment variables matter.",
    "sources": [
      "primary"
    ]
  },
  {
    "kicker": "SECTION 08",
    "title": "Deployment and Live Project",
    "subtitle": "The final project was pushed to GitHub and deployed for free on Render as a live Node web service.",
    "expectedVisual": "Deployment story cards from repo to cloud URL.",
    "cards": [
      [
        "Repository",
        "The deployment pipeline starts from the `badash01/miniproject` GitHub repository and tracks the latest master branch."
      ],
      [
        "Hosting",
        "Render free tier runs the Node backend and serves the web experience publicly with automatic redeploys after pushes."
      ],
      [
        "Live URL",
        "Public website: miniproject-rjwq.onrender.com. Free instances may sleep, so the first response can take longer."
      ]
    ],
    "notes": "Mention that deployment makes the project demonstration-ready and not just a local prototype.",
    "sources": [
      "repo",
      "live"
    ]
  },
  {
    ...
    "subtitle": "The project already solves a real study problem, and it can grow into a much stronger academic or production research assistant.",
    "expectedVisual": "Closing metric cards for growth directions.",
    "metrics": [
      [
        "OCR",
      ...
      [
        ...
        "Summarise and contrast papers together"
      ],
      [
        "Chat",
        "Ask questions about papers",
        "Turn summary into interactive learning"
      ]
    ],
    "notes": "Close by saying the project combines AI usefulness, web engineering, browser automation, and cloud deployment in one practical mini-project.",
    "sources": [
      "primary"
    ]
  }
...
async function addPlate(slide, slideNo, opacityPanel = false) {
  ...
  if (await pathExists(platePath)) {
    ...
    addShape(slide, "rect", 0, 0, W, H, PAPER, TRANSPARENT, 0, { slideNo, role: "fallback background" });
    addShape(slide, "ellipse", 914, -130, 400, 400, "#27C47D14", TRANSPARENT, 0, { slideNo, role: "background orb" });
    addShape(slide, "ellipse", -90, 515, 260, 260, "#E86F5B12", TRANSPARENT, 0, { slideNo, role: "background orb" });
    addShape(slide, "roundRect", 916, 88, 250, 80, "#FFFFFFAA", INK, 1, { slideNo, role: "background chip" });
    addText(slide, slideNo, "AI + Web + Browser Extension", 942, 118, 200, 22, {
      size: 14,
      color: ACCENT_DARK,
      bold: true,
      face: MONO_FACE,
      checkFit: false,
      role: "background microcopy",
    });
  }
  if (opacityPanel) {
...
function addReferenceCaption(slide, slideNo) {
  addText(
    slide,
    slideNo,
    "Research Paper Summariser | MCA project presentation",
    64,
    674,
    980,
    22,
...
async function slideCover(presentation) {
  ...
  addShape(slide, "roundRect", 810, 144, 340, 286, WHITE, INK, 1.2, { slideNo, role: "cover system panel" });
  addText(slide, slideNo, "Project Snapshot", 842, 170, 210, 24, {
    size: 14,
    color: ACCENT_DARK,
    bold: true,
    face: MONO_FACE,
  ...
  addMetricCard(slide, slideNo, 842, 212, 120, 138, "2", "Interfaces", "Web + extension", ACCENT);
  addMetricCard(slide, slideNo, 980, 212, 138, 138, "1", "Backend", "Secure key usage", GOLD);
  addShape(slide, "roundRect", 842, 366, 276, 44, "#F0FBF6", INK, 1, { slideNo, role: "live tag" });
  addText(slide, slideNo, "Live on Render free deployment", 866, 380, 224, 18, {
    size: 15,
    color: INK,
    face: BODY_FACE,
    role: "cover live tag",
